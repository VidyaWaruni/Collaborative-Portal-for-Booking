...
<div class="row" style="margin-right: 0px;margin-bottom: 200px">
    <div class="col-md-4 col-sm-4 col-xs-4" >
        @foreach($edit_banquets as $banquet)
        <form action="{{route('post_edit_banquet',['id'=>$banquet->id])}}" method="post" style="padding-left: 30px;padding-top: 40px" enctype="multipart/form-data">
            <div class="form-group">
                <label for="name" style="font-size: 11px" class="label-control">Banquet Name</label>
                <input  style="font-size: 10px; @if($errors->has('banquet_name'))border-color: red @endif " type="text" class="form-control form-control-sm " id="banquet_name" name="banquet_name" value="{{$banquet->banquet_name}}"  placeholder="">
                @if($errors->has('banquet_name'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('banquet_name')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="restaurant_address" style="font-size: 11px">Banquet Address</label>
                <input  style="font-size: 10px; @if($errors->has('banquet_address'))border-color: red @endif " type="text" class="form-control form-control-sm" id="banquet_address" name="banquet_address" value="{{$banquet->banquet_address}}" placeholder="" >
                @if($errors->has('banquet_address'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('banquet_address')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="contact" style="font-size: 11px">Banquet Contact Number</label>
                <input  style="font-size: 10px; @if($errors->has('banquet_contact'))border-color: red @endif " type="text" class="form-control form-control-sm" id="banquet_contact" name="banquet_contact"  value="{{$banquet->banquet_contact}}" placeholder="" >
                @if($errors->has('banquet_contact'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('banquet_contact')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="contact" style="font-size: 11px">Banquet Price</label>
                <input  style="font-size: 10px; @if($errors->has('banquet_price'))border-color: red @endif " type="text" class="form-control form-control-sm" id="banquet_price" name="banquet_price"  value="{{$banquet->banquet_price}}" placeholder="" >
                @if($errors->has('banquet_price'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('banquet_price')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="contact" style="font-size: 11px">Banquet Capacity</label>
                <input  style="font-size: 10px; @if($errors->has('banquet_capacity'))border-color: red @endif " type="text" class="form-control form-control-sm" id="banquet_capacity" name="banquet_capacity"  value="{{$banquet->max_Capacity}}" placeholder="" >
                @if($errors->has('banquet_capacity'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('banquet_capacity')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="contact" style="font-size: 11px">Banquet Image</label>
                <input type="file" style="font-size: 10px; @if($errors->has('banquet_image'))border-color: red @endif "  id="banquet_image" name="banquet_image" class="form-control form-control-sm"  value="{{$banquet->image}}">
                @if($errors->has('banquet_image'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('banquet_image')}}</span>
                @endif
            </div>
            <input type="hidden" name="banquet_category" value="banquets">
            <button type="submit" class="btn btn-default button-thumbnail" >UPDATE BANQUET</button>
    ...
    <div class="col-md-8 col-sm-8 col-xs-8" style="padding-top: 30px;padding-left: 100px;padding-right: 70px">
        @if($count > 3)
            <form action="{{route('search_results',['name'=>'banquets'])}}" method="get">
                <div class="input-group mb-3" style="width: 60%;margin-left: 160px;margin-top: 40px;padding-bottom: 40px" type="text">
                    <input style="font-size: 11px" type="text" id="search-data"  name="search-data"   class="form-control form-control-sm" placeholder="Search your banquets by banquet name or address" aria-label="Recipient's username" aria-describedby="basic-addon2" required>
        ...
        @if($count == 0)
            <h6 style="color: red;text-align: center;margin-top: 180px">You have no banquets</h6>
        @endif
        <div class="text-thumbnail" style="margin-left: 300px;margin-bottom: 50px;">{{ $banquets->links() }}</div>
        @foreach($banquets as $banquet)
            <div class="img-thumbnail wall-thumbnail" style="padding-bottom: 18px;padding-left:5px;padding-right:20px;padding-top:1px;margin-top: 10px;">
                <a href="{{route('delete_banquet',['id'=>$banquet->id])}}" style="color: firebrick;" ><i style="font-size:17px;margin-top: 10px;color: firebrick;cursor: pointer" class="fa float-right text-thumbnail">&#xf00d;</i></a>
                <div class="media p-3">

                    <img src="uploads/{{$banquet->image}}" alt="Upload Your Hotel Image " class="mr-3 mt-3 img-thumbnail img-responsive" style="width:350px;height: 200px">
                    ...
                    <div class="img-thumbnail" style="padding-bottom: 20px;padding-left:20px;padding-right:56px;margin-top: 19px;background-color:#F3F9FE ">
                        <div class="media-body" style="padding-top: 20px">
                            ...
                            <h4 style="text-transform: uppercase;"><b>{{str_limit($banquet->banquet_name, $limit = 10, $end = '...')}}</b></h4>
                            <small style="text-transform: capitalize;">{{$banquet->banquet_address}}</small><br/>
                            <small>{{$banquet->banquet_contact}}</small><br/>

                            <a href="{{route('set_available',['id'=>$banquet->id])}}"><button style="margin-top: 36px;font-size: 11px" type="button" class="btn btn-info btn-sm"><b>AVAILABILITY</b></button></a>
                            <a href="{{route('edit_banquet',['id'=>$banquet->id])}}"><button style="margin-top: 36px;font-size: 11px" type="button" class="btn btn-dark btn-sm"><b>UPDATE</b></button></a>
                            <a href="{{route('view_gallery',['id'=>$banquet->id])}}"><button style="margin-top: 36px;font-size: 11px;background-color: #191970;color: white" type="button" class="btn  btn-sm"><b>GALLERY</b></button></a>
                ...
                <table style="margin-left: 20px">
                    ...
                    @foreach($availables as $available)
                        @if($available->pro_id == $banquet->id)
                            <tr>
        ...
        @endforeach
        <div class="text-thumbnail" style="margin-left: 300px;margin-top: 50px;">{{ $banquets->links() }}</div>
    </div>
</div>
