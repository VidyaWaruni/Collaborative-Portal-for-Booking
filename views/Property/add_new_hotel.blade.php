...
<div class="row" style="margin-right: 0px;margin-bottom: 200px">
    <div class="col-md-4 col-sm-4 col-xs-4" >
        <form action="{{route('post_add_hotel')}}" method="post" style="padding-left: 30px;padding-top: 40px" enctype="multipart/form-data">
            <div class="form-group">
                <label for="name" style="font-size: 11px" class="label-control">Hotel Name</label>
                <input  style="font-size: 10px; @if($errors->has('name'))border-color: red @endif " type="text" class="form-control form-control-sm " id="name" name="name" value="{{Request::old('name') ?: ''}}"  placeholder="Hotel Name">
                @if($errors->has('name'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('name')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="address" style="font-size: 11px">Hotel Address</label>
                <input  style="font-size: 10px; @if($errors->has('address'))border-color: red @endif " type="text" class="form-control form-control-sm" id="address" name="address" value="{{Request::old('address') ?: ''}}" placeholder="Hotel Address" >
                @if($errors->has('address'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('address')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="contact" style="font-size: 11px">Hotel Contact Number</label>
                <input  style="font-size: 10px; @if($errors->has('contact'))border-color: red @endif " type="text" class="form-control form-control-sm" id="contact" name="contact"  value="{{Request::old('contact') ?: ''}}" placeholder="Hotel Contact" >
                @if($errors->has('contact'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('contact')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="contact" style="font-size: 11px">Hotel Image</label>
                <input type="file" style="font-size: 10px; @if($errors->has('hotel_image'))border-color: red @endif "  id="hotel_image" name="hotel_image" class="form-control form-control-sm"  value="{{Request::old('hotel_image') ?: ''}}">
                @if($errors->has('hotel_image'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('hotel_image')}}</span>
                @endif
            </div>
            <input type="hidden" name="category" value="hotels">
            <button type="submit" class="btn btn-default button-thumbnail" >ADD HOTEL</button>
    ...
    <div class="col-md-8 col-sm-8 col-xs-8" style="padding-top: 30px;padding-left: 100px;padding-right: 70px">
        @if($count > 3)
            <form action="{{route('search_results',['name'=>'hotels'])}}" method="get">
                <div class="input-group mb-3" style="width: 60%;margin-left: 160px;margin-top: 40px;padding-bottom: 40px" type="text">
                    <input style="font-size: 11px" type="text" id="search-data"  name="search-data"   class="form-control form-control-sm" placeholder="Search your hotels by hotel name or address" aria-label="Recipient's username" aria-describedby="basic-addon2" required>
    ...
    @if($count == 0)
            <h6 style="color: red;text-align: center;margin-top: 180px">You have no hotels</h6>
        @endif
            <div class="text-thumbnail" style="margin-left: 300px;margin-bottom: 50px;">{{ $hotels->links() }}</div>
            @foreach($hotels as $hotel)
                <div class="img-thumbnail wall-thumbnail" style="padding-bottom: 18px;padding-left:5px;padding-right:20px;padding-top:1px;margin-top: 10px;">
                    <a href="{{route('delete_hotel',['id'=>$hotel->id])}}" style="color: firebrick;" ><i style="font-size:17px;margin-top: 10px;color: firebrick;cursor: pointer" class="fa float-right text-thumbnail">&#xf00d;</i></a>
                <div class="media p-3">

                <img src="uploads/{{$hotel->image}}" alt="Upload Your Hotel Image " class="mr-3 mt-3 img-thumbnail img-responsive" style="width:350px;height: 200px">
                ...
                <div class="media-body" style="padding-top: 20px">
                    ...
                    <h4 style="text-transform: uppercase;"><b>{{$hotel->hotel_name}}</b></h4>
                    <small style="text-transform: capitalize;">{{$hotel->hotel_address}}</small><br/>
                    <small>{{$hotel->hotel_contact}}</small><br/>

                    <a href="{{route('set_available',['id'=>$hotel->id])}}"><button style="margin-top: 36px;font-size: 11px" type="button" class="btn btn-info btn-sm"><b>AVAILABILITY</b></button></a>
                    <a href="{{route('edit_hotel',['id'=>$hotel->id])}}"><button style="margin-top: 36px;font-size: 11px" type="button" class="btn btn-dark btn-sm"><b>UPDATE</b></button></a>
                    <a href="{{route('view_gallery',['id'=>$hotel->id])}}"><button style="margin-top: 36px;font-size: 11px;background-color: #191970;color: white" type="button" class="btn  btn-sm"><b>GALLERY</b></button></a>
                    ...
                    <table style="margin-left: 20px">
                        ...
                        @foreach($availables as $available)
                            @if($available->pro_id == $hotel->id)
                                <tr>
                                    <td><small >Set date to available : </small></td>
                                    <td style="padding-left: 20px"><small >{{$available->available_date}}</small></td>
                                    <td style="padding-left: 40px"><small >Day Single : </small></td>
                                    <td style="padding-left: 20px"><small >{{$available->daySingle}} </small></td>
                                    <td style="padding-left: 40px"><small >Day Double : </small></td>
                                    <td style="padding-left: 20px"><small >{{$available->dayDouble}} </small></td>
            ...
            @endforeach
        <div class="text-thumbnail" style="margin-left: 300px;margin-top: 50px;">{{ $hotels->links() }}</div>
    </div>
</div>
