...
<div class="row" style="margin-right: 0px">
    @foreach($edit_apartments as $edit_apartment)
    <div class="col-md-4 col-sm-4 col-xs-4" >
        <form action="{{route('post_edit_apartment',['id'=>$edit_apartment->id])}}" method="post" style="padding-left: 30px;padding-top: 40px" enctype="multipart/form-data">
            <div class="form-group">
                <label for="name" style="font-size: 11px" class="label-control">Apartment Name</label>
                <input  style="font-size: 10px; @if($errors->has('name'))border-color: red @endif " type="text" class="form-control form-control-sm " id="name" name="name" value="{{$edit_apartment->apartment_name}}"  placeholder="Apartment Name">
                @if($errors->has('name'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('name')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="address" style="font-size: 11px">Apartment Address</label>
                <input  style="font-size: 10px; @if($errors->has('address'))border-color: red @endif " type="text" class="form-control form-control-sm" id="address" name="address" value="{{$edit_apartment->apartment_address}}" placeholder="Apartment Address" >
                @if($errors->has('address'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('address')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="contact" style="font-size: 11px">Apartment Contact Number</label>
                <input  style="font-size: 10px; @if($errors->has('contact'))border-color: red @endif " type="text" class="form-control form-control-sm" id="contact" name="contact"  value="{{$edit_apartment->apartment_contact}}" placeholder="Apartment Contact" >
                @if($errors->has('contact'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('contact')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="contact" style="font-size: 11px">Apartment price</label>
                <input  style="font-size: 10px; @if($errors->has('price'))border-color: red @endif " type="text" class="form-control form-control-sm" id="price" name="price"  value="{{$edit_apartment->apartment_price}}" placeholder="Apartment Price" >
                @if($errors->has('price'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('price')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="contact" style="font-size: 11px">Apartment Type</label>
                <input  style="font-size: 10px; @if($errors->has('type'))border-color: red @endif " type="text" class="form-control form-control-sm" id="type" name="type"  value="{{$edit_apartment->apartment_type}}" placeholder="Apartment Type" >
                @if($errors->has('type'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('type')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="contact" style="font-size: 11px">Apartment Details</label>
                <textarea  style="font-size: 10px; @if($errors->has('details'))border-color: red @endif " class="form-control form-control-sm" rows="5" id="details" name="details" value="{{$edit_apartment->apartment_details}}" placeholder="Apartment Details"></textarea>
                @if($errors->has('details'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('details')}}</span>
                @endif
            </div>
            <div class="form-group ">
                <label for="contact" style="font-size: 11px">Apartment Image</label>
                <input type="file" style="font-size: 10px; @if($errors->has('apartment_image'))border-color: red @endif "  id="apartment_image" name="apartment_image" class="form-control form-control-sm"  value="{{$edit_apartment->image}}">
                @if($errors->has('apartment_image'))
                    <span style="color: red;font-size: 11px" class="help-block">{{$errors->first('apartment_image')}}</span>
                @endif
            </div>
            <input type="hidden" name="category" value="apartments">
            <button type="submit" class="btn btn-default button-thumbnail" >UPDATE APARTMENT</button>
    ...
    <div class="col-md-8 col-sm-8 col-xs-8" style="padding-top: 30px;padding-left: 100px;padding-right: 70px">
        @if($count > 3)
            <form action="{{route('search_results',['name'=>'apartments'])}}" method="get">
                <div class="input-group mb-3" style="width: 60%;margin-left: 160px;margin-top: 40px;padding-bottom: 40px" type="text">
                    <input style="font-size: 11px" type="text" id="search-data"  name="search-data"   class="form-control form-control-sm" placeholder="Search your apartments by apartment name or address" aria-label="Recipient's username" aria-describedby="basic-addon2" required>
        ...
        @if($count == 0)
            <h6 style="color: red;text-align: center;margin-top: 180px">You have no apartments</h6>
        @endif
        <div class="text-thumbnail" style="margin-left: 300px;margin-bottom: 50px;">{{ $apartments->links() }}</div>
        @foreach($apartments as $apartment)
            <div class="img-thumbnail wall-thumbnail" style="padding-bottom: 18px;padding-left:5px;padding-right:20px;padding-top:1px;margin-top: 10px;">
                <a href="{{route('delete_apartment',['id'=>$apartment->id])}}" style="color: firebrick;" ><i style="font-size:17px;margin-top: 10px;color: firebrick;cursor: pointer" class="fa float-right text-thumbnail">&#xf00d;</i></a>
                <div class="media p-3">

                    <img src="uploads/{{$apartment->image}}" alt="Upload Your Hotel Image " class="mr-3 mt-3 img-thumbnail img-responsive" style="width:350px;height: 200px">
                    ...
                    <div class="img-thumbnail" style="padding-bottom: 20px;padding-left:20px;padding-right:56px;margin-top: 19px;background-color:#F3F9FE ">
                        <div class="media-body" style="padding-top: 20px">
                            ...
                            <h4 style="text-transform: uppercase;"><b>{{$apartment->apartment_name}}</b></h4>
                            <small style="text-transform: capitalize;">{{$apartment->apartment_address}}</small><br/>
                            <small>{{$apartment->apartment_contact}}</small><br/>

                            <a href="{{route('set_available',['id'=>$apartment->id])}}"><button style="margin-top: 36px;font-size: 11px" type="button" class="btn btn-info btn-sm"><b>AVAILABILITY</b></button></a>
                            <a href="{{route('edit_apartment',['id'=>$apartment->id])}}"><button style="margin-top: 36px;font-size: 11px" type="button" class="btn btn-dark btn-sm"><b>UPDATE</b></button></a>
                            <a href="{{route('view_gallery',['id'=>$apartment->id])}}"><button style="margin-top: 36px;font-size: 11px;background-color: #191970;color: white" type="button" class="btn  btn-sm"><b>GALLERY</b></button></a>
        ...
        @endforeach
        <div class="text-thumbnail" style="margin-left: 300px;margin-top: 50px;">{{ $apartments->links() }}</div>
    </div>
</div>
